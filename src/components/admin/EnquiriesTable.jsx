  ...
  return (
    <div>
      <div className="flex flex-wrap gap-4">
            ...
            {STATUS_OPTIONS.map((s) => (
              <button
                ...
                className={cn(
                  ...
                  status === s ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 text-gray-600 hover:border-gray-300"
                )}
              >
                {s}
              </button>
        ...
        <div>
          <label className="mb-1 block text-xs font-medium uppercase tracking-wide text-gray-400">Source</label>
          <div className="flex gap-1.5">
            {SOURCE_OPTIONS.map((s) => (
              <button
                ...
                className={cn(
                  "rounded-full border px-3 py-1.5 text-xs font-medium capitalize transition-colors",
                  source === s ? "border-gray-900 bg-gray-900 text-white" : "border-gray-200 text-gray-600 hover:border-gray-300"
                )}
              >
                {s}
              </button>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((e) => (
                <tr key={e.id}>
                  ...
                  <td className="px-4 py-3 capitalize text-gray-600">{e.tripType || "—"}</td>
                  <td className="px-4 py-3 text-gray-600">
                    {[e.pickup, e.destination || e.drop].filter(Boolean).join(" → ") || "—"}
                  </td>
                  <td className="px-4 py-3 capitalize text-gray-600">{e.source}</td>
