...
export function ContactForm() {
  ...
  if (status === "success") {
    return (
      <div className="glass-light rounded-[var(--radius-lg)] p-8 text-center shadow-[var(--shadow-soft)]">
  ...
  return (
    <form onSubmit={onSubmit} className="glass-light rounded-[var(--radius-lg)] p-6 shadow-[var(--shadow-soft)] sm:p-8">
      ...
      {status === "error" && (
        <p className="mt-3 text-sm text-[var(--color-danger)]">Something went wrong — please call or WhatsApp us instead.</p>
      )}
    </form>
  );
}
